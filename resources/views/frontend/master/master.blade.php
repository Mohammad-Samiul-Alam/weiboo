...
<body>
    ...
    <script>
        $('.searchInput1class').click(function(){
            var search_input = $('.search-input').val();
            var category_id = $(this).val();
            var color_id = $(this).val();
            var link = "{{route('shop')}}" + "?q=" + search_input + "&category_id=" + color_id;
            window.location.href = link;
        })

        // $('.')
        // $('.searchInput1class').(function(e) {
        //     var  vlue = $('.search-input').val();
        //     alert(vlue);
        // })
...
<!-- Mirrored from reactheme.com/products/html/weiboo/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Sat, 11 Mar 2023 06:47:21 GMT -->
...
{{-- $('.category_id').click(function() {
    ...
    var min = $('.leftLabel').val();
    var max = $('.rightLabel').val();
    var link = "{{route('shop')}}" + "?q=" + search_input + "&min=" + min + "&max=" + max + "&sort=" + sort + "&brand_id=" + brand_id + "&category_id=" + color_id  + "&color_id=" + color_id;
    window.location.href = link;
}) --}}
